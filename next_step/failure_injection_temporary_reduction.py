...
time_of_injection = 10000#5000
final_time_of_injection = time_of_injection+10000
...
for i, key in enumerate(keys_of_interest):
    for j in range(len(dataset[key])):
        if j > time_of_injection and j < final_time_of_injection:
...
joblib.dump(dataset, 'dataset_temporary_reduction.joblib')